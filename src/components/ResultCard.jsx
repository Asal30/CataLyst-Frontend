      ...
      <div className="relative z-10 space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="w-full md:w-64 bg-slate-800/80 rounded-2xl p-4 border border-slate-700">
            ...
            <p className="text-right mt-1 text-[10px] text-slate-400 font-medium">Level: <span className="text-slate-300">{result.confidence_level}</span></p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-slate-400">Highlighted areas show regions the AI focused on while making its decision.</p>

            <div className="relative aspect-[4/3] rounded-2xl overflow-hidden border border-slate-700/50 shadow-lg group">
              <img src={`http://127.0.0.1:8000${result.gradcam_url}`} alt="Grad-CAM Analysis" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                <p className="text-xs text-white font-medium bg-black/50 backdrop-blur-md px-3 py-1 rounded-full border border-white/10">Visualizing affected regions</p>
              </div>
            </div>
          </div>
        </div>
