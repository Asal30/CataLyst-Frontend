export default function ResultCard({ result }) {
  ...
  return (
    <div className="animate-fade-in duration-700 bg-slate-900/80 backdrop-blur-md rounded-3xl p-6 md:p-8 border border-slate-700/50 relative overflow-hidden group">
      ...
      {/* Glow Effect */}
      <div className={`absolute top-0 right-0 w-64 h-64 rounded-full blur-[80px] opacity-20 pointer-events-none -translate-y-1/2 translate-x-1/2 ${isCataract ? 'bg-red-500' : 'bg-emerald-500'}`}></div>
      ...
      <div className="relative z-10 space-y-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex-1">
              <p className="text-slate-400 text-xs font-bold tracking-wider uppercase mb-2 flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${isCataract ? 'bg-red-500' : 'bg-emerald-500'} animate-pulse`}></span>
                Diagnosis
              </p>
            ...
            <div className="w-full md:w-64 bg-slate-800/80 rounded-2xl p-4 border border-slate-700">
               ...
               <p className="text-right mt-1 text-[10px] text-slate-500 font-medium">
                  Level: <span className="text-slate-300">{result.confidence_level}</span>
               </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             ...
             <div className="space-y-4">
               <h3 className="text-sm font-semibold text-slate-200 flex items-center gap-2">
                 <div className="p-1.5 bg-blue-500/10 rounded-lg text-blue-400">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01"></path></svg>
                 </div>
                 AI Analysis
               </h3>
             ...
             <div className="space-y-4">
               ...
               <div className="relative rounded-2xl overflow-hidden border border-slate-700/50 group/img h-48 md:h-full min-h-[12rem] shadow-lg">
                  <img
                    src={`http://127.0.0.1:8000${result.gradcam_url}`}
                    alt="Grad-CAM Analysis"
                    className="w-full h-full object-cover transition-transform duration-700 group-hover/img:scale-110"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-transparent to-transparent opacity-0 group-hover/img:opacity-100 transition-opacity duration-300 flex items-end p-4">
